...
class LivenessService:
    ...
    @staticmethod
    def get_details(signals: Dict[str, Any]) -> Dict[str, Any]:
        """
        Возвращает полную информацию о liveness check.

        Args:
            signals: Dict с сигналами liveness

        Returns:
            Dict с score, risk, passed и исходными сигналами
        """
        result = LivenessService.fuse(signals)
        return {
            "score": result["score"],
            "risk": result["risk"],
            "passed": result["score"] >= settings.LIVENESS_THRESHOLD,
            "signals": signals
        }